...
- (void) editAction
{
    ...
    if (app.loginType == YES) {
        ...
        SheQuVC *VC = [SheQuVC new];
        
        VC.block = ^(NSError *err){
            NSLog(@"回来了");
            if (err) {
                //
                
                
                UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"发表失败" preferredStyle:UIAlertControllerStyleAlert];
